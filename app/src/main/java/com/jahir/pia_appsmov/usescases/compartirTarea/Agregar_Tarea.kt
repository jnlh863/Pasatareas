...
class Agregar_Tarea : AppCompatActivity() {
    ...
    private fun subirArchivo(pdf_url: Uri?) {
        ...
        reference.putFile(pdf_url!!).addOnSuccessListener (OnSuccessListener{
                try{
                    ...
                    Toast.makeText(this@Agregar_Tarea, "Seleccón exitosa", Toast.LENGTH_SHORT)
                    .show()
    ...
    private fun CompartirTarea(nom: String, mat: String, des: String, u: String?) {
        ...
        this.miTarea.userN = u.toString()
        this.miTarea.nameH = nom
        this.miTarea.materiaH = mat
        this.miTarea.DH = des
        this.miTarea.url = URLH.toString()

        CoroutineScope(Dispatchers.IO).launch {
            val call = RetrofitClient.webService.guardarTarea(miTarea)
            runOnUiThread{
                if(call.isSuccessful){
                    Toast.makeText(this@Agregar_Tarea, call.body().toString(),Toast.LENGTH_SHORT).show()
                    limpiarObjeto()
